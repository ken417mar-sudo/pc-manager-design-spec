...
**所有新增与修改必须严格按本规范与 Token 实现。** 颜色、字体、圆角、间距、阴影、组件状态等一律使用 `tokens.css` 中的变量；若 Token 未覆盖，须**用户单独确认**后再补充到 `tokens.css` / `spec.md` / `resources/component-states.json`，不得自行发明新值。详见 `documentation/SPEC-CONSTRAINTS.md`。
...
- **顶栏**：高 64px，内边距 20px 25px；操作区 gap 16px；右上工具栏图标 16×16（tool-* 为 SVG + **二级图标色** `--color-icon-secondary`）、图标间距 20px；登录区 gap 6px、文案 14px/22px、三级色；分割线 1×14px、`--color-divider`。
- **防御卡片**（374×460，left 60 top 20）：
  - 标题栏：相对卡片 (1,0) 宽 372 高 40，内边距 0 12px；标题 12px/550、三级色、**图标 14×14 使用 PNG**（`icon-shield@2x.png`）、gap 6。
  ...
  - 操作区：相对卡片 (0,404) 宽 374 高 20，gap 44、padding 0 30；项 12/550、一级色、**图标 16×16 使用 PNG**（`icon-scan@2x.png`、`icon-quarantine@2x.png`、`icon-trust@2x.png`）、项内 gap 8。
...
- **导航图标**：仅保留 SVG（`nav-device.svg` … `nav-store.svg`），PNG 可按需用 export-slices 重新导出。
- `604:2916` icon/头像/切图 → `user-avatar@2x.png`, `user-avatar@3x.png`
- **tool-* 工具栏图标（右上操作栏）**：与 icon-* 一致，按层导出为 SVG（`scripts/export-icon-svg.mjs`），见 `assets/slices/tool-grid.svg`、`tool-dropdown.svg`、`tool-minimize.svg`、`tool-close.svg`。
...
- **icon-* 图标**：**安全系统（防御卡片）** 标题与操作区四枚用 PNG（`icon-shield`、`icon-scan`、`icon-quarantine`、`icon-trust` 的 @2x/@3x，由 export-slices 导出）。弹窗/浏览器卡片标题仍用 SVG（`icon-popup.svg`、`icon-browser.svg`）+ `--color-text-tertiary`。
- `603:1718` image/保护状态/切图：本仓库使用分解图层（browser-ring-*、browser-center、browser-shield 等），不保留复合图；如需复合图可用 export-slices 按节点 603:1718 导出。
...
## 导出脚本
本仓库已包含主工程的切图导出脚本（`scripts/export-slices.mjs`），输出到本仓库 `assets/slices/`。icon-* 与 tool-* 图标使用 `scripts/export-icon-svg.mjs` 导出 SVG（currentColor + token 着色）。
